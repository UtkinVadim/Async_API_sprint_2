...
expected_film_detailed = {'id': 'a7b11817-205f-4e1a-98b5-e3c48b824bc3', 'title': 'Star Trek', 'imdb_rating': 6.4,
                          ...
                          'genre': ['Mystery', 'Sci-Fi', 'Adventure'], 'director': 'Robert Wise',
                          'actors': [{'id': '5a3d0299-2df2-4070-9fda-65ff4dfa863c', 'name': 'Leonard Nimoy'},
                          ...
                          'writers': [{'id': '317ba074-e7bf-4c7a-a6e3-da71b1e7cbf2', 'name': 'Alan Dean Foster'},
...
expected_films_genre_filter_sorted = [
    {'id': '0312ed51-8833-413f-bff5-0e139c11264a', 'title': 'Star Wars: Episode V - The Empire Strikes Back',
     'imdb_rating': 8.7},
    {'id': '3d825f60-9fff-4dfe-b294-1a45fa1e115d', 'title': 'Star Wars: Episode IV - A New Hope', 'imdb_rating': 8.6},
...
expected_films_genre_filter_sorted_asc = [
...
expected_films_all = [
    ...
    {'id': '516f91da-bd70-4351-ba6d-25e16b7713b7', 'title': 'Star Wars: Episode III - Revenge of the Sith',
     'imdb_rating': 7.5},
    {'id': 'c4c5e3de-c0c9-4091-b242-ceb331004dfd', 'title': 'Star Wars: Episode II - Attack of the Clones',
     'imdb_rating': 6.5}, {'id': '4af6c9c9-0be0-4864-b1e9-7f87dd59ee1f', 'title': 'Star Trek', 'imdb_rating': 7.9},
    ...
    {'id': '46f15353-2add-415d-9782-fa9c5b8083d5', 'title': 'Star Wars: Episode IX - The Rise of Skywalker',
     'imdb_rating': 6.7}, {'id': 'fda827f8-d261-4c23-9e9c-e42787580c4d', 'title': 'A Star Is Born', 'imdb_rating': 7.7},
    {'id': '57beb3fd-b1c9-4f8a-9c06-2da13f95251c', 'title': 'Solo: A Star Wars Story', 'imdb_rating': 6.9},
...
expected_film_filter_cache = {'result': [
...
@pytest.mark.asyncio
async def test_film_detailed(make_get_request, es_client):
    response = await make_get_request(method='/film/a7b11817-205f-4e1a-98b5-e3c48b824bc3')
    assert response.status == http.HTTPStatus.OK
    assert len(response.body) == len(expected_film_detailed)
    assert response.body == expected_film_detailed


@pytest.mark.asyncio
async def test_film_detailed_not_found(make_get_request, es_client):
    response = await make_get_request(method='/film/Nonexistent_film')
    assert response.status == http.HTTPStatus.NOT_FOUND


@pytest.mark.asyncio
async def test_film_filter(make_get_request, es_client):
    response = await make_get_request(method='/film',
                                      params={"filter_genre_id": "ca88141b-a6b4-450d-bbc3-efa940e4953f",
                                              'page[size]': '5'
                                              })
    assert response.status == http.HTTPStatus.OK
...
@pytest.mark.asyncio
async def test_film_filter_sorted_desc(make_get_request, es_client):
    response = await make_get_request(method='/film',
                                      params={"id": "ca88141b-a6b4-450d-bbc3-efa940e4953f",
                                              'page[size]': '5',
                                              'sort': '-imdb_rating'})
    assert response.status == http.HTTPStatus.OK
    assert len(response.body) == len(expected_films_genre_filter_sorted)
    assert response.body == sorted(expected_films_genre_filter_sorted, key=lambda x: x['imdb_rating'], reverse=True)


@pytest.mark.asyncio
async def test_film_filter_sorted_asc(make_get_request, es_client):
    response = await make_get_request(method='/film',
                                      params={"id": "ca88141b-a6b4-450d-bbc3-efa940e4953f",
                                              'page[size]': '3',
                                              'sort': 'imdb_rating'})
    assert response.status == http.HTTPStatus.OK
    assert len(response.body) == len(expected_films_genre_filter_sorted_asc)
    assert response.body == sorted(expected_films_genre_filter_sorted_asc, key=lambda x: x['imdb_rating'],
                                   reverse=False)


@pytest.mark.asyncio
async def test_film_filter_paginator(make_get_request, es_client):
    response = await make_get_request(method='/film',
                                      params={"id": "ca88141b-a6b4-450d-bbc3-efa940e4953f",
...
@pytest.mark.asyncio
async def test_film_filter_all(make_get_request, es_client):
    response = await make_get_request(method='/film',
                                      params={'page[size]': '9999'})
    assert response.status == http.HTTPStatus.OK
    assert len(response.body) == len(expected_films_all)
...
async def test_film_filter_all(make_get_request, es_client, redis_client):
    ...
    assert data_json == expected_film_filter_cache
