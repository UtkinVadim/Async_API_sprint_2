...
async def test_find_person_from_redis_cache(make_get_request, redis_client):
    personfilms = [
        Film(id="1", title="Хоббит: Нежданное путешествие", imdb_rating=9, role="Хоббит"),
        Film(id="2", title="Хоббит: Пустошь Смауга", imdb_rating=8, role="Хоббит"),
        Film(id="3", title="Хоббит: Битва пяти воинств", imdb_rating=10, role="Хоббит")
    ]
    person = Person(id="hobbit_id", fullname="Bilbo Baggins", film_ids=personfilms)
    await redis_client.set(f"person::{person.id}", person.json())
