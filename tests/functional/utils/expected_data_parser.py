...
class ExpectedDataParser:
    def __init__(self):
        self.parser = DataParser()

    async def get_expected_genres_data(self, genre_id: str = None) -> Union[list, dict, str]:
        genres_data_from_file = await self.parser.get_data_from_file(file_name="genres_data.json")
        expected_data = [
    ...
    async def get_expected_persons_data(self, person_id: str = None):
        persons_data_from_file = await self.parser.get_data_from_file(file_name="persons_data.json")
        expected_data = [
            {"uuid": person.get("id"),
    ...
    async def get_film_detailed_data(self, film_id: str):
        films_data_from_file = await self.parser.get_data_from_file(file_name="films_data.json")
    ...
    async def get_film_data(self, genre_id: str = None, page_size: int = None, sort_by: str = None):
        ...
        if genre_id:
            genre = await self.get_expected_genres_data(genre_id)
